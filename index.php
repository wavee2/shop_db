...
if (isset($_GET['remove1'])) {
   ...
   mysqli_query($conn, "DELETE FROM `short_urls` WHERE id = '$remove_id'") or die('query failed');
   header('location:index.php');
}
...
<body>
   ...
   <div class="container">
      ...
      <div class="user-profile">
         ...
         <div class="flex">
            ...
            <form action="" method="post">
               ...
               <input type="submit" name="add_to_url" value="Generate Short URL">
            </form>
            <br>
            <?php

            if (isset($_POST['add_to_url'])) {
               ...
               if (mysqli_stmt_execute($stmt)) {
                  ...
                  $qrCodePath = "images_qrcode/{$shortCode}.png";
                  QRcode::png("localhost/shopping cart with user login/{$shortCode}", $qrCodePath, QR_ECLEVEL_L, 10);
      ...
      <div class="shopping-cart">
         ...
         <h1 class="heading">Short urls</h1>
         ...
         <table>
            ...
            <tbody>
               <?php
               $cart_query = mysqli_query($conn, "SELECT * FROM `short_urls` ") or die('query failed');
               if (mysqli_num_rows($cart_query) > 0) {
                  while ($fetch_cart = mysqli_fetch_assoc($cart_query)) {
               ?>
                     <tr>
                        <td><?php echo $fetch_cart['id']; ?></td>
                        <td><?php echo $fetch_cart['short_code']; ?></td>
                        <td><?php echo $fetch_cart['full_url']; ?></td>
                        <td><?php echo $fetch_cart['visit_count']; ?></td>
                        <td><a href="index.php?remove1=<?php echo $fetch_cart['id']; ?>" class="delete-btn" onclick="return confirm('remove url?');">remove</a></td>
                     </tr>
               <?php
                  }
               }
